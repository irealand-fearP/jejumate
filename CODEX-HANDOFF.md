...
1. ~~홈 화면이 잠금 이미지(PNG) 기반이라 동적 데이터가 홈에 안 보임~~ → **완료(커밋 2426dfb)**: 홈을 실제 컴포넌트(MeetingTimeline/MeetingCard/AskEntryCard/BoardSection/BottomNav)로 분리하고 `/api/home` 실데이터 렌더, 하단 내비 잘림 CSS(fixed→sticky, dvh/overflow) 수정. 신청·RAG·온보딩 시트 실브라우저 회귀 통과.
2. 시드 카테고리 균형(맛집/숙소 데이터 보강).
3. 제품 방향(소유자 결정): 카톡 데이터는 초반 유입용이며, 유입 후엔 서비스 내 작성 글로만 운영 예정.
4. **(보류 중인 제품 결정)** 코덱스 중단 시점 사본에 있던 "정책 탭 → 생활(board) 탭 교체 + BoardScreen 신규" 방향 전환은 이식하지 않았다(/board 라우트 부재로 죽은 링크 위험). 진행하려면 라우트·API까지 세트로 만들 것.
