...
## 다음 시작 지점 (Day 3 잔여)
1. **OPENAI_API_KEY 대기 중**(여전히 미발급): 키가 `backend/.env`에 들어오면 `llm_provider`/`embedding_provider`를 "openai"로 전환 → 실제 응답 검증 → **기존 시드 53건 재임베딩** → RAG 관련성 재확인(UX P0).
2. **시안 채택 대기**: 사용자가 1/2/3 중 고르면 dev가 프론트에 톤 반영(구조·카피는 세 시안 공통이라 CSS/레이아웃 중심 작업).
3. 리허설 + git push(대표), 발표자료(plans/2026-07-07-발표자료-초안.md) 최종화.
