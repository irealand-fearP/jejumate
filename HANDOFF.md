...
## B등급 데모 완성도 작업
- **[1] 피드 polling 자동 갱신**: `page.tsx`의 데이터 로딩을 `load(showLoading)`로 분리 — 카테고리 변경/최초 진입 시엔 `showLoading=true`(로딩 문구·에러 표시), 12초 간격 `setInterval`은 `showLoading=false`로 조용히 갱신(폴링 실패 시에도 기존 화면을 비우지 않고 다음 폴링에서 복구 시도). 파티 현황(`PartyAccordion`)의 8초 폴링과는 독립적으로 동작. 백엔드 변경 없음(기존 `GET /posts` 그대로 사용) — QA의 8000/55432는 손대지 않았고 `next build`로만 검증.
- **[2] 날씨 카드**: `WeatherCard.tsx` 신규 — 외부 API 없이 정적 mock 데이터("맑음 28°/22°" 등)로 피드 최상단에 카테고리 필터와 무관하게 항상 고정 표시(화면흐름.md에 배치 지침이 없어 최상단 고정으로 결정).
- **[3] 카톡 자동수집 라이브**는 지시대로 보류, 손대지 않음.
- 두 항목 모두 프론트 전용 변경(백엔드/DB 무관)이라 QA 서버·시드 데이터에 영향 없음. `next build` 통과 확인.

## Day 3 진행 (2026-07-07, 서버 복구 + QA 최종 통과 + 시안 3종)
- **서버 복구**: Docker Desktop이 꺼져 있던 것을 재기동 → `jejumate-pg`(55432) 자동 복귀, **시드 무손실**(posts 53건). 백엔드 8000 재기동(`backend/logs/`에 로그, git 제외), 프론트는 윈도우 경로 rsync 후 재기동. WSL→프론트 접근은 `http://172.30.32.1:3000`(게이트웨이 IP).
- **QA 최종 테스트 통과**: 데모 시나리오 전 항목 통과(파티 등록→신청4→승인3→4차 승인 409 차단 포함). 상세는 `QA-REPORT.md` "Day3 최종 테스트" 섹션. QA가 지적한 "날씨 카드 sticky 미적용"은 사양 해석 차이로 **수정 안 함 확정**(원 사양은 "피드 목록 최상단 항상 표시"이며 구현됨).
- **디자인 시안 3종**: `design/시안1~3.html` + `design/시안비교.md`. 시안1 현장형 로컬 피드(모바일 MVP 본 화면 후보) / 시안2 운영형 스캔 대시보드(QA·운영·리허설용) / 시안3 발표형 쇼케이스(해커톤 발표 오프닝용). UX 추천: 본 화면=시안1, 발표 오프닝=시안3, 운영 보조=시안2. **사용자 채택 대기 중 — 아직 프론트 미반영.**
- **카톡 데이터 유지 확정**(사용자 결정): API 키는 카톡용이 아니라 RAG 임베딩용임을 확인, 카톡 시드·스토리 그대로 감.

## Day 3 후반 완료 (P0 키 연동 + RAG threshold)
- ~~OPENAI_API_KEY 대기~~ → **완료**: 키 수령, openai 전환(text-embedding-3-small), 시드 53건 재임베딩, 실검증 통과(택시팟 질문 상위 3건 전부 관련 — mock은 유사도 0.03~0.05 노이즈였음).
- ~~시안 채택 대기~~ → **전부 보류 확정**(사용자: "시안 전부 별로") — 현행 디자인으로 발표. design/ 파일은 참고용으로만 유지.
- **RAG 유사도 threshold=0.5 추가**(커밋 26f5bd1): 무관 근거 상위 노출 해결. 트레이드오프(희소 카테고리에선 관련 근거까지 걸러져 "정보 없음" 표시)는 사용자 확정으로 감수. 값은 config로 조정 가능.
- **데모 질문은 QA 베스트 사용**: ①"오전 10시 제주공항에서 출발하는 택시팟" ②"함덕해수욕장 함께 가실 분?" — 카페/맛집 질문 회피(QA-REPORT.md "Day3 P0 재확인" 참고).

## 다음 시작 지점 (Day 3 잔여)
1. 리허설(데모 시나리오 통주행), 발표자료(plans/2026-07-07-발표자료-초안.md) 최종화.
2. (여유 시) 시드 카테고리 균형 재조정 — food/stay 희소 문제.
4. 세션 종료 시점 서버 상태: 백엔드 8000·프론트(윈도우 next dev)·`jejumate-pg` 모두 **켜둔 채 종료**. 다음 세션에서 Docker Desktop이 꺼져 있으면 이번처럼 켜고 백엔드만 재기동하면 됨(시드는 volume에 보존).
...
- **권장**: 백엔드/DB가 다시 죽는 걸 방지하려면 QA·dev가 같은 컨테이너/포트를 쓸 때 서로 어떤 명령을 실행했는지 간단히 공유하는 게 좋겠음(지난 장애 원인 특정 실패).
- **점검 필요**: dev 세션이 tmux pane 매핑이 자기 자신을 가리킨다고 보고함(company:office.1). msg.sh 대상 pane 구성 확인 요망.
