...
def test_excludes_closed_and_expired_posts(db_session):
    ...
    results = search_posts(db_session, query_embedding=[0.0] * 1536, now=NOW)
...
def test_category_filter_applied(db_session):
    ...
    results = search_posts(db_session, query_embedding=[0.0] * 1536, category="food", now=NOW)
...
def test_orders_by_cosine_similarity_descending(db_session):
    ...
    results = search_posts(db_session, query_embedding=close_vector, now=NOW)
...
def test_respects_top_k_limit(db_session):
    ...
    results = search_posts(db_session, query_embedding=[0.0] * 1536, top_k=2, now=NOW)
